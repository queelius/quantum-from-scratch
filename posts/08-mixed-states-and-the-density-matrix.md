...
```
    ...
    rho for |+>:
     [[0.5+0.j 0.5+0.j]
     [0.5+0.j 0.5+0.j]]
    diagonal (probabilities): [0.5 0.5]
...
```
    ...
    one qubit of a Bell pair:
     [[0.5+0.j 0. +0.j]
     [0. +0.j 0.5+0.j]]
    identical to the classical mixture: True
...
```
    ...
    purity of the Bell pair (whole): 1.0
    purity of one qubit (reduced):   0.5
...
![png](08-mixed-states-and-the-density-matrix_files/08-mixed-states-and-the-density-matrix_13_0.png)
...
```
    ...
    expectation <Z> via Tr(Z rho): 0.5
    matches cos(pi/3):             0.5
